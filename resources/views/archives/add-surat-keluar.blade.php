...
<div class="container">
    ...
    <form class="form-horizontal" method="POST" action="{{ url('/archive/surat-keluar') }}" enctype="multipart/form-data">
        {{ csrf_field() }}
        ...
        <div class="form-group{{ $errors->has('nomor_surat') ? ' has-error' : '' }}">
            ...
            <div class="col-md-6">
                <input id="nomor_surat" type="text" class="form-control" name="nomor_surat" value="{{ old('nomor_surat') }}" autofocus>
        ...
        <div class="form-group{{ $errors->has('ket_surat_keluar') ? ' has-error' : '' }}">
            <label for="ket_surat_keluar" class="col-md-4 control-label">Keterangan Surat</label>

            <div class="col-md-6">
                <!-- <textarea id="ket_surat_keluar" class="form-control" name="ket_surat_keluar" required></textarea> -->
                <div class="input-group mb-3">
                  <select class="custom-select" id="inputGroupSelect01" name="ket_surat_keluar">
                    @foreach($ket_surat_keluar as $keterangan)
                        <option value="{{ $keterangan['id'] }}">{{ $keterangan['title'] }}</option>
                    @endforeach
                  </select>
                </div>
